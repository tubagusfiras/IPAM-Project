...
def _build_block_section(block: dict, allocs: list, theme: dict, is_first: bool = True) -> str:
  ...
  <div class="cards">
    ...
    <div class="card"><div class="label">Free</div><div class="value" style="color:#22c55e">{free}</div></div>
...
async def export_block_pdf(block_id: str, db=Depends(get_db)):
    row = await db.fetchrow(BLOCK_QUERY, block_id)
    if not row:
    ...
    allocs_list = [dict(a) for a in allocs]
    block_dict = dict(row)
    html = _build_pdf_html(block_dict, allocs_list)
...
async def export_summary_pdf(db=Depends(get_db)):
    all_blocks = await db.fetch(ALL_BLOCKS_QUERY)
    all_list = [dict(b) for b in all_blocks]
    html = _build_summary_pdf_html(all_list)
    pdf_bytes = WeasyprintHTML(string=html).write_pdf()
    return StreamingResponse(
        io.BytesIO(pdf_bytes),
        media_type="application/pdf",
