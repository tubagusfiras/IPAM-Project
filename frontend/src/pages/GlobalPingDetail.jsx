...
export default function GlobalPingDetail({ onNavigate }) {
  ...
  const oracleSgHistory = history.filter(h => h.source === "oracle_sg");
  const oracleUsHistory = history.filter(h => h.source === "oracle_us");

  // Helper render region status
  const renderRegionCard = (label, status, accent) => {
    const isOnline = status === "online";
    const isOffline = status === "offline";
    const isPending = !status || status === "pending";
    return (
      <Card accent={isOnline ? "#22c55e" : isOffline ? "#ef4444" : "#64748b"} padding={16}>
        ...
        <div style={{ fontSize: 22, fontWeight: 700, color: isOnline ? "var(--success)" : isOffline ? "var(--danger)" : "var(--text-dim)" }}>
          {isOnline ? <span style={{display:"inline-flex",alignItems:"center",gap:4}}><svg viewBox="0 0 24 24" fill="none" stroke="var(--success)" strokeWidth="2.5" width="16" height="16"><polyline points="20 6 9 17 4 12"/></svg>Online</span>
           : isOffline ? <span style={{display:"inline-flex",alignItems:"center",gap:4}}><svg viewBox="0 0 24 24" fill="none" stroke="var(--danger)" strokeWidth="2.5" width="16" height="16"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>Offline</span>
           : <span style={{display:"inline-flex",alignItems:"center",gap:4,fontSize:14}}><svg viewBox="0 0 24 24" fill="none" stroke="var(--text-dim)" strokeWidth="2" width="14" height="14"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>N/A</span>}
        </div>
        <div style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 2 }}>{isPending ? "Agent belum lapor" : ""}</div>
      </Card>
    );
  };

  const renderHistoryBars = (hist, label) => (
    <div style={{ marginBottom: 12 }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: "var(--text-muted)", marginBottom: 6 }}>
        {label} — {hist.filter(h => h.status === "online").length}/{hist.length} online
      </div>
      <div style={{ display: "flex", gap: 3, alignItems: "flex-end", height: 32 }}>
        ...
        {hist.length === 0 && <span style={{ fontSize: 10, color: "var(--text-dim)" }}>No data</span>}
      </div>
    </div>
  );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
        <Card accent={data.icmp_status === "online" ? "#22c55e" : "#ef4444"} padding={16}>
          <div style={{ fontSize: 10, fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>ICMP (Local Server)</div>
        ...
        <Card accent={data.http_status === "online" ? "#22c55e" : data.http_status === "offline" ? "#ef4444" : "#f59e0b"} padding={16}>
          <div style={{ fontSize: 10, fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>HTTP (Global)</div>
        ...
        {renderRegionCard("Oracle Cloud SG", data.oracle_sg_status)}
        {renderRegionCard("Oracle Cloud US", data.oracle_us_status)}
      ...
      <div className="card" style={{ padding: 16 }}>
        ...
        {history.length === 0 ? (
          ...
          <>
            {renderHistoryBars(icmpHistory, "ICMP (Local Server)")}
            {renderHistoryBars(httpHistory, "HTTP (Global)")}
            {renderHistoryBars(oracleSgHistory, "Oracle Cloud SG")}
            {renderHistoryBars(oracleUsHistory, "Oracle Cloud US")}
          </>
        )}
      </div>
      ...
      <div className="card" style={{ padding: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 4, fontSize: 12 }}>
          {[
            ...
            ["Oracle SG Status", data.oracle_sg_status || "pending"],
            ["Oracle US Status", data.oracle_us_status || "pending"],
